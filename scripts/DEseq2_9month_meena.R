...
# read in counts data
counts_data <- read.csv("GSE76205_GeneLevel_Raw_data_MEedited_9month.csv", row.names = 1)
head(counts_data)
...
colData <- read.csv("9month_metadata_MEedited_Rinputfile.csv", row.names = 1)
View (colData )
...
#save raw results
write.csv(res, file = "9monthCSvsControl_differential_expression_allresults.csv")
...
# Save results
write.csv(significant_results, file = "9monthCSvsControl_differential_expression_significantresultsonly.csv")
